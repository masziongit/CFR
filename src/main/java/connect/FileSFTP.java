...
public class FileSFTP {
    ...
    public FileSFTP(Properties prop) {
        ...
        try {
            ...
            logger.info("Tranfer file..");
            String[] prefixs = prop.getProperty("file.name.prefix").split(Constant.Regex.ARRAY);

            for (String prefix : prefixs) {
                String fileName = prefix + dateStr + "." + prop.getProperty("file.name.type");
                try {
                    //download
                    sftpChannel.get(prop.getProperty("sftp.download.path") + fileName, fileName);
                    logger.debug("Download from " + prop.getProperty("sftp.download.path") + fileName);
                    //upload
            ...
            logger.info("Tranfer file complete!!");
            sftpChannel.exit();
